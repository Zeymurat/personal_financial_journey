...
const AddInvestmentModal: React.FC<AddInvestmentModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    symbol: '',
    name: '',
    type: 'currency' as 'currency' | 'gold' | 'preciousMetal' | 'fund' | 'stock' | 'crypto',
  ...
  // Modal açıldığında form'u sıfırla ve varsayılan tipi "Döviz" yap
  useEffect(() => {
    if (isOpen) {
      setFormData({
        symbol: '',
  ...
  const handleValueSelect = (code: string, name: string) => {
    ...
    // Yatırım türüne göre güncel fiyatı otomatik doldur
    switch (formData.type) {
      case 'stock':
      ...
      case 'gold':
      case 'preciousMetal':
        // Döviz, altın ve değerli metaller için güncel fiyatı al
  ...
  // Modal açıldığında formu sıfırla (bu useEffect kaldırıldı, yukarıdaki useEffect kullanılıyor)
  ...
  // TransactionType (alış/satış) değiştiğinde fiyatı güncelle (döviz/kripto için)
  useEffect(() => {
  ...
  }, [formData.transactionType, formData.symbol, formData.type, allCurrencies]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.symbol || !formData.name) {
      toast.error(t('addModal.selectInvestment'));
      return;
    }
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-8">
          <div>
            <h3 className="text-2xl font-black text-slate-900 dark:text-white">{t('addModal.title')}</h3>
            <p className="text-slate-500 dark:text-slate-400 mt-1">{t('addModal.subtitle')}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <span className={`font-semibold ${formData.transactionType === 'buy'
                    ? 'text-green-700 dark:text-green-300'
                    : 'text-slate-200 dark:text-slate-600 hover:text-slate-300 dark:hover:text-slate-500'
                }`}>{t('types.buy')}</span>
              </button>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              >
                ...
                <span className={`font-semibold ${formData.type === 'gold'
                    ? 'text-yellow-700 dark:text-yellow-300'
                    : 'text-slate-200 dark:text-slate-600 hover:text-slate-300 dark:hover:text-slate-500'
                }`}>{t('types.gold')}</span>
              </button>
              ...
              <button
                ...
                className={`p-3 rounded-xl border-2 transition-all duration-200 text-sm ${
                  formData.type === 'crypto'
                    ? 'border-yellow-500 bg-yellow-50 dark:bg-yellow-500/20 text-yellow-700 dark:text-yellow-300'
                    : 'border-slate-200 dark:border-slate-600 hover:border-slate-300 dark:hover:border-slate-500'
                }`}
              >
                ...
                <span className={`font-semibold ${formData.type === 'crypto'
                    ? 'text-yellow-700 dark:text-yellow-300'
                    : 'text-slate-200 dark:text-slate-600 hover:text-slate-300 dark:hover:text-slate-500'
                }`}>{t('types.crypto')}</span>
              </button>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">
                {formData.transactionType === 'buy' ? t('addModal.priceBuy') : t('addModal.priceSell')}
              </label>
              <div className="relative">
                <input
                  type="number"
