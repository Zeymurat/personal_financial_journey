...
const EditTransactionModal: React.FC<EditTransactionModalProps> = ({
  ...
  // transaction değiştiğinde form'u doldur
  useEffect(() => {
    if (transaction) {
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updateData = {
        ...
        amountInTRY: amountInTRY // Güncel kur ile hesaplanmış TL karşılığı
      };

      await transactionAPI.update(transaction.id, updateData);
      
      // Güncellenmiş transaction'ı parent'a gönder
      ...
      alert('İşlem güncellenirken bir hata oluştu. Lütfen tekrar deneyin.');
    }
  };
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-8">
          <div>
            <h3 className="text-2xl font-black text-slate-900 dark:text-white">İşlem Düzenle</h3>
            <p className="text-slate-500 dark:text-slate-400 mt-1">İşlem bilgilerini güncelleyin</p>
          </div>
          <button
            onClick={onClose}
