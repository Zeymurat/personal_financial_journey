      ...
      >
        ...
        >
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('shared:fundSelection.title')}</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('shared:fundSelection.subtitle')}</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-6">
            {(() => {
              const filteredFunds = allFunds.filter((fund) => {
                ...
                return fund.key.toLowerCase().includes(query) || fund.value.toLowerCase().includes(query);
              });
                ...
                return (
                  <div className="col-span-full text-center py-8">
                    <p className="text-gray-500 dark:text-gray-400">
                      {t('shared:fundSelection.noResults', { query: searchQuery })}
                    </p>
                  </div>
                );
              }
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        <span className="text-base font-bold text-gray-900 dark:text-white block">{fund.key}</span>
                        <span className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2">{fund.value}</span>
                      </div>
                      <input
                        type="checkbox"
                        checked={isSelected}
