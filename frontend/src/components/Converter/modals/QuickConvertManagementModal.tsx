...
const QuickConvertManagementModal: React.FC<QuickConvertManagementModalProps> = ({
  ...
  const handleAddConvert = async () => {
    ...
    if (from === to) {
      toast.error(t('toast.sameCurrency'));
      return;
    }
    ...
    >
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('modalQuick.title')}</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('modalQuick.subtitle')}</p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                {t('modalQuick.fromCurrencyLabel')}
              </label>
              <select
                id="newConvertFrom"
        ...
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{t('modalQuick.listTitle')}</h3>
          {quickConverts.length === 0 ? (
            <p className="text-gray-500 dark:text-gray-400 text-center py-4">{t('modalQuick.listEmpty')}</p>
          ) : (
              ...
              {quickConverts.map((convert) => (
                ...
                >
                  ...
                  <button
                    ...
                    title={t('modalQuick.removeTitle')}
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
